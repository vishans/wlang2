...
exercise:
    EXERCISE IDENTIFIER SETS INTEGER_LITERAL REPS INTEGER_LITERAL WEIGHT STRING '{' set_details '}' { 
        $$ = new Exercise(
    ...
    | EXERCISE IDENTIFIER SETS INTEGER_LITERAL REPS INTEGER_LITERAL WEIGHT STRING { 
        $$ = new Exercise(
            std::string($2),                // Convert name to std::string
            std::stoi($4),                  // Convert sets to int
            std::stoi($6),                  // Convert reps to int
...
rep_details:
    rep_details rep_detail { $$ = $1; $1->push_back($2); }
    | rep_detail { $$ = new std::vector<RepDetail*>(); $$->push_back($1); }
    ;

rep_detail:
    REP INTEGER_LITERAL WEIGHT STRING { 
        $$ = new RepDetail(std::stoi($2), $4, *new std::map<std::string, std::string>(), *new std::map<std::string, std::string>(), *new std::map<std::string, std::string>()); 
    }
    | REPS INTEGER_LITERAL '-' INTEGER_LITERAL WEIGHT STRING { 
        $$ = new std::vector<RepDetail*>();
        for (int i = $2; i <= $4; ++i) {
            $$->push_back(new RepDetail(i, $6, *new std::map<std::string, std::string>(), *new std::map<std::string, std::string>(), *new std::map<std::string, std::string>()));
        }
    }
    | REP INTEGER_LITERAL WEIGHT STRING custom_fields {
        ...
        $$ = new RepDetail($2, $4, combinedFields, *new std::map<std::string, std::string>(), *new std::map<std::string, std::string>());
    }
    | REPS INTEGER_LITERAL '-' INTEGER_LITERAL WEIGHT STRING custom_fields {
        ...
        for (int i = $2; i <= $4; ++i) {
            $$->push_back(new RepDetail(i, $6, combinedFields, *new std::map<std::string, std::string>(), *new std::map<std::string, std::string>()));
        }
    }
    ;
